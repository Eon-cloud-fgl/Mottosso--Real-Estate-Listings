...
function Buttons({ onShowAdd, onShowModify, onDeleteItem, isDeleteDisabled }) {
  return (
    <>
      <button className="buttons" onClick={onShowAdd} id="addProduct">
        Agregar Producto
      </button>
      <button className="buttons" onClick={onShowModify} id="modifyProduct">
        Modificar Producto
      </button>
      <button
        className="buttons"
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "/api/controller/estateController.php",
        formData,
        {
          headers: {
      ...
      toast.error(
        "Error al agregar la propiedad. Inténtalo de nuevo más tarde."
      );
  ...
  return (
    <div className="form-overlay" onClick={onClose}>
      ...
      >
        ...
        <label>
          Zona:
          <select name="area" required defaultValue="elegi la zona">
...
function ModifyProduct({ onClose, estate, onUpdate }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(
        "Error al modificar la propiedad. Inténtalo de nuevo más tarde."
      );
      ...
      >
        ...
        <label>
          ...
          <input
            ...
            name="listing_id"
            required
            defaultValue={estate.listing_id}
          />
        </label>
        ...
        <label>
          Zona:
          <select name="area" required defaultValue={estate.area}>
            <option value="puerto">puerto</option>
            <option value="sur">sur</option>
            <option value="centro">centro</option>
          </select>
        ...
        <label>
          ...
          <input
            ...
            name="covered_area"
            min="0"
            defaultValue={estate.covered_area}
          />
        </label>
        ...
        <label>
          ...
          <input
            ...
            name="total_area"
            min="0"
            defaultValue={estate.total_area}
          />
        </label>
        ...
        <div className="gallery-preview">
          ...
          {estate.property_images &&
            estate.property_images.map((img) => (
        ...
        <button type="submit" disabled={loading}>
          {loading ? "Modificando..." : "Modificar Propiedad"}
        </button>
      </form>
    </div>
  );
...
function Item({ estate, onSelect, selected }) {
  ...
  return (
    <div className={`item ${selected ? "selected" : ""}`} onClick={onSelect}>
      ...
      <h2>{estate.title}</h2>
      <p>
        {estate.type} en {estate.operation}
      </p>
      <p>
      ...
      <p>
        <strong>
          ${estate.price.toLocaleString()} {estate.currency}
        </strong>
      </p>
      <p
        className={`status ${
          estate.status === "activo" ? "active" : "inactive"
        }`}
      >
        {estate.status}
      </p>
...
export default function Dashboard() {
  ...
  const handleDeleteItem = () => {
    ...
    const confirmDelete = window.confirm(
      "¿Estás seguro de que quieres desactivar este inmueble?"
    );
    if (!confirmDelete) return;

    // Realiza la solicitud para eliminar el producto
    axios
      .post("/api/controller/estateController.php", {
        action: "deleteEstate",
        id: selectedItemId,
      })
      .then((response) => {
      ...
      .catch((error) => {
        ...
        toast.error(
          "Error al eliminar el producto. Inténtalo de nuevo más tarde."
        );
      });
  };
  useEffect(() => {
    axios
      .get("/api/controller/dashboard_session.php", {
