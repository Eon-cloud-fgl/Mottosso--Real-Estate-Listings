        ...
        <div className="ce-item-content">
          <div className="ce-item-info">
            ...
            <div className="ce-info--icon">
              <span>
                <RxHome />{" "}
                {estate.total_area && estate.total_area !== 0
                  ? `${estate.total_area}m² terreno`
                  : "Sin info"}
              </span>
              <span>
                <RxRulerSquare />{" "}
                {estate.covered_area && estate.covered_area !== 0
                  ? `${estate.covered_area}m² cubiertos`
                  : "Sin info"}
              </span>
              <span>
                <RxDimensions />{" "}
                {estate.rooms && estate.rooms !== 0
                  ? `${estate.rooms} ambientes`
                  : "Sin info"}
              </span>
              <span>
                <PiToiletPaper />{" "}
                {estate.bathrooms && estate.bathrooms !== 0
                  ? `${estate.bathrooms} baños`
                  : "Sin info"}
              </span>
            </div>
...
function Filter({ onFilterChange }) {
  ...
  return (
    <div className="ce-filter">
      <div className="ce-filter--search" style={{ position: "relative" }}>
        <input
          type="text"
          value={query}
      ...
      <div className="ce-filter--options">
        ...
        <button onClick={handleAddEmployee}>
          <AiFillFilter /> Filtros
        </button>

        {showForm && (
          <Filters onClose={handleCloseForm} onFilterChange={onFilterChange} />
        )}
      </div>
    </div>
  );
...
export default function Estate() {
  ...
  return (
    <div className="ce-container">
      ...
      <div className="ce-items">{content}</div>
    </div>
  );
}
